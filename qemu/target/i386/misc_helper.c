...
target_ulong helper_read_crN(CPUX86State *env, int reg)
{
    ...
    // Unicorn: call registered read_crn hooks
    HOOK_FOREACH_VAR_DECLARE;
    HOOK_FOREACH(env->uc, hook, UC_HOOK_INSN) {
        if (hook->to_delete)
            continue;
...
void helper_write_crN(CPUX86State *env, int reg, target_ulong t0)
{
    ...
    // Unicorn: call registered write_crn hooks
    HOOK_FOREACH_VAR_DECLARE;
    HOOK_FOREACH(env->uc, hook, UC_HOOK_INSN) {
        if (hook->to_delete)
            continue;
...
void helper_wrmsr(CPUX86State *env)
{
    ...
    HOOK_FOREACH(env->uc, hook, UC_HOOK_INSN) {
        ...
        // Multiple wrmsr callbacks returning different values is undefined.
        if (hook->insn == UC_X86_INS_WRMSR)
            skip_write = ((uc_cb_insn_gen_t)hook->callback)(env->uc, hook->user_data);
...
void helper_rdmsr(CPUX86State *env)
{
   ...
   // Unicorn: call registered RDMSR hooks
    ...
    HOOK_FOREACH(env->uc, hook, UC_HOOK_INSN) {
        ...
        // Multiple rdmsr callbacks returning different values is undefined.
        if (hook->insn == UC_X86_INS_RDMSR)
            skip_read = ((uc_cb_insn_gen_t)hook->callback)(env->uc, hook->user_data);
